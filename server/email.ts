/**
 * Email for the Literal Memories pricing calculator — PurelyMail over SMTP.
 *
 * History: Manus build sent via nodemailer + PurelyMail (this template). On
 * 2026-05-28 it was stubbed to throw, for a Cloudflare Workers deploy that was
 * abandoned; the app then shipped to Render free, which blocks SMTP ports, so
 * no mail was sent. Restored 2026-09-15 for the move to NODE-01, where LC Portal
 * and ScriptRipper already send through PurelyMail. Template recovered from
 * git (5f6afd9~1:server/email.ts).
 *
 * Changes from the original:
 *  - The T-Mobile email-to-SMS send is removed: tmomail.net is discontinued.
 *  - Each send is independent. The owner copy goes first, so Joel hears about
 *    an order even if the customer copy fails; its failure is logged only.
 *  - The customer copy decides success: it throws if it fails, and the caller
 *    records emailSent = failed and the site does not claim it was sent.
 *  - User-typed text is HTML-escaped before it enters the template.
 *
 * Env: SMTP_HOST (smtp.purelymail.com), SMTP_PORT (465 SSL/TLS or 587 STARTTLS),
 * SMTP_USER (joel@literalmemories.com; also the From address, which must be on
 * the PurelyMail domain for DMARC), SMTP_PASS (mailbox or App Password).
 */
...
function getTransporter() {
  ...
  return nodemailer.createTransport({
    host,
    port,
    // 465 is implicit TLS; 587 upgrades via STARTTLS. PurelyMail offers both.
    secure: port === 465,
    auth: { user, pass },
  });
}
...
/** Every string that came from the form, escaped before it reaches HTML. */
function escapePayload(p: EstimateEmailPayload): EstimateEmailPayload {
  return {
    ...p,
    clientName: escapeHtml(p.clientName),
    clientEmail: escapeHtml(p.clientEmail),
    clientPhone: escapeOptional(p.clientPhone),
    projectNotes: escapeOptional(p.projectNotes),
    tier: escapeHtml(p.tier),
    lineItems: p.lineItems.map(i => ({ ...i, category: escapeHtml(i.category), name: escapeHtml(i.name) })),
    deliveryItems: p.deliveryItems.map(d => ({ ...d, name: escapeHtml(d.name) })),
  };
}
...
function buildHtmlEmail(payload: EstimateEmailPayload, isOwner: boolean): string {
  const greeting = isOwner
    ...
    : `<p>Thank you, ${payload.clientName}. We've received your estimate request, and our team will be in touch shortly. Here is a copy of your estimate for your records.</p>`;
...
export async function sendEstimateEmail(payload: EstimateEmailPayload): Promise<void> {
  ...
  const from = `"Literal Memories" <${process.env.SMTP_USER}>`;
  const safe = escapePayload(payload);
  const subject = `Literal Memories — Estimate for ${payload.clientName}`;

  // 1. Owner copy first. A failure here is logged, not thrown.
  try {
    ...
    console.error("[Email] Owner copy failed:", err);
  }

  // 2. Customer copy. Its result is what the site reports.
  await transporter.sendMail({
    from,
    to: payload.clientEmail,
    replyTo: OWNER_EMAIL,
    subject,
    html: buildHtmlEmail(safe, false),
  });
}
